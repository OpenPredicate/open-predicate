...
| [**SQL++ / N1QL**](https://www.couchbase.com/sqlplusplus/) | The closest existing thing to this specification's ambition — a SQL-like language over JSON, backed by an [academic formal semantics](https://arxiv.org/pdf/1405.3631) rather than prose alone. Its `ANY`/`EVERY`/`SATISFIES` land on the same vacuous-truth answer as `$some`/`$every` (§5.8): `EVERY` over an empty collection is TRUE, independently arrived at. Its NULL/MISSING distinction is arguably four-valued where [§4.1](./SPEC.md#41-three-valued-logic) is three — `NOT NULL` stays `NULL` and `NOT MISSING` stays `MISSING`, so the *cause* of UNKNOWN survives negation, where here it has to be recovered separately via `$exists`/`$isNull` ([§4.2](./SPEC.md#42-missing-versus-null)). Its `WITHIN` (unknown-depth descent) and array slicing are exactly what [§3.2](./SPEC.md#32-path-grammar)'s dropped wildcard segment declines to add, for the same reason. What it confirms is that the semantics here are not novel — SQL++ reached most of the same answers — and that what's actually missing from the field is not another dialect but an open governance model: it is a single vendor's language, unlike this one. |
